...
export async function withRunLogging<T>(
  ...
  await pool.query(
    `INSERT INTO agent_runs (id, agent_name, status, trigger, started_at)
     VALUES ($1, $2, 'running', $3, $4)`,
    [id, agentName, trigger, startedAt]
  )

  try {
    const result = await fn(id)
    await pool.query(
      `UPDATE agent_runs SET status = 'success', finished_at = $1 WHERE id = $2`,
      [Date.now(), id]
    )
    return result
  } catch (err) {
    await pool.query(
      `UPDATE agent_runs SET status = 'error', error_msg = $1, finished_at = $2 WHERE id = $3`,
      [String(err), Date.now(), id]
    )
    throw err
  }
}
